...
set.seed(101)
#setwd("~/Documents/2019coevolution_geno_pheno/code_and_data_paper3")
setwd("/home/agarcia337/Documents/ML_Meyer/2021coevolution_geno_pheno/Leapfrog_code_and_data_paper")

################ bacteria and virus mutation profile
#importing it from html output
...
# bac_table = data.frame(bac_table_[, 1:2], an_b = rep("", nrow(bac_table)), bac_table[, 3:ncol(bac_table)])
# importing a csv
bac_table_doc<-read.csv("bacteria_mut_FINAL.csv",header=TRUE)
# create a new column called an_b (for the ancestor)
bac_table = data.frame(bac_table_doc[, 1:2], an_b = rep("", nrow(bac_table_doc)), bac_table_doc[, 3:ncol(bac_table_doc)])
...
#why they deleted this mut
#bac_table = bac_table[-1, ]
# retrieve the snp data only and convert it to binary
bac_feature_raw = bac_table[, 4:53] 
bac_feature_binary = ifelse(bac_feature_raw == "" | bac_feature_raw == "?", 0, 1)
#bac_feature_binary = ifelse(bac_feature_raw == "", 0, 1)
bac_table_b = data.frame(bac_table, init_appear_day = get_initial_appear(bac_feature_binary), nonsyn_mutation = check_nonsyn(bac_table[, 54]))
bac_feature_binary_add_an = bac_feature_binary    #only add genome but not mutation/ancestor indicator
bac_feature_binary<-bac_feature_binary[-1]
...
#######################################
# fraction of co-occurring mutations
#######################################
library(ComplexHeatmap)
library(circlize)
# col_fun = colorRamp2(c(0,0.001,0.5), c("lightgrey1", "gold", "red3"))
col_fun = colorRamp2(c(0,0.001,0.5), c("grey87", "lightblue", "magenta4"))
cocurr_mut_1<-matrix(0,nrow(phage_feature_binary),nrow(phage_feature_binary))
for (m in 1:(nrow(phage_feature_binary)-1)){
  for(mm in (m+1):nrow(phage_feature_binary)){
    tmp<-phage_feature_binary[m,]+phage_feature_binary[mm,]
    cocurr_mut_1[m,mm]<-sum(tmp==2)
    cocurr_mut_1[mm,m]<-cocurr_mut_1[m,mm]
  }
}
Heatmap(cocurr_mut/ncol(bac_feature_binary), name = "Co-occurrence of mutations", col = col_fun, column_title = "mutations number",row_dend_reorder = FALSE,column_dend_reorder = FALSE)

h1<-Heatmap(cocurr_mut_1/ncol(phage_feature_binary), name = "Co-occurrence of mutations", column_title = "phage mutations",col = col_fun,cluster_rows=F,cluster_columns=F, row_dend_reorder = FALSE,column_dend_reorder = FALSE,border = TRUE,rect_gp  = gpar(col= "white",lwd = 1), width = ncol(cocurr_mut_1)*unit(2, "mm"), 
            height = nrow(cocurr_mut_1)*unit(2, "mm"))
h<-Heatmap(cocurr_mut/ncol(bac_feature_binary), name = "Co-occurrence of mutations", column_title = "host mutations",col = col_fun,cluster_rows=F,cluster_columns=F, row_dend_reorder = FALSE,column_dend_reorder = FALSE,border = TRUE,rect_gp  = gpar(col= "white",lwd = 2),width = ncol(cocurr_mut_1)*unit(1, "mm"), 
           height = nrow(cocurr_mut_1)*unit(1, "mm"))
Heatmap(cocurr_mut/ncol(bac_feature_binary), name = "Co-occurrence of mutations", column_title = "mutations number",col = "BuPu", cluster_rows=F,cluster_columns=F, row_dend_reorder = FALSE,column_dend_reorder = FALSE,border = TRUE,rect_gp  = gpar(col= "white",lwd = 2))
#######################################
# Create a random matrix that conserves the residuals but looses the correlation.
#######################################
# take original mutation matrix- ** feautre_binary and select two random isolated strains and 2 random mutations and do a permutation.
...
# importing a csv
phage_table_doc<-read.csv("phage_mut_FINAL.csv",header=TRUE)
phage_table = data.frame(phage_table_doc[, 1:2], an_p = rep("", nrow(phage_table_doc)), phage_table_doc[, 3:ncol(phage_table_doc)])

# convert SNP to binary
phage_feature_raw = phage_table[, 4:47]
...
##################################################################
################ phenotypes load (infection matrix)###############
##################################################################
...
#using eop as phenotype. Create a matrix that is (number of bact) by (number of phage)
...
phage_cor_res = get_phylo_cor_res(pheno_matrix_add_an_ordered, phage_duplicated_genomes_mapping) #all phages have positive corr
...
#bac nonsyn mutation and all genome 16 by 51 (this might vary on the number of NSyn mut you considered. I neglected the unknowns)
...
#phage nonsyn mutation and all genome 61 by 45 (this might vary on the number of NSyn mut you considered. I neglected the unknowns)
